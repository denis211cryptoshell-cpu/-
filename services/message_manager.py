...
class MessageManager:
    ...
    async def get_last_section(self, user_id: int) -> str | None:
        """
        Получить ключ последнего открытого раздела.

        Args:
            user_id: Telegram ID пользователя

        Returns:
            Ключ раздела (например, 'about') или None
        """
        row = await db_adapter.fetchone(
            "SELECT last_section FROM users WHERE telegram_id = ?",
            user_id,
        )
        return row[0] if row else None

    async def set_last_section(self, user_id: int, section_key: str) -> None:
        """
        Сохранить ключ последнего открытого раздела.

        Args:
            user_id: Telegram ID пользователя
            section_key: Ключ раздела (например, 'about')
        """
        await db_adapter.execute(
            "UPDATE users SET last_section = ? WHERE telegram_id = ?",
            section_key,
            user_id,
        )

    async def clear_last_section(self, user_id: int) -> None:
        """
        Очистить ключ последнего открытого раздела.

        Args:
            user_id: Telegram ID пользователя
        """
        await db_adapter.execute(
            "UPDATE users SET last_section = NULL WHERE telegram_id = ?",
            user_id,
        )
